...
class RedirectedSignup(AuthSignupHome):
    ...
    def get_auth_signup_qcontext(self):
        ...
        other_fields = {k: v for (k, v) in request.params.items() if k in ['last_name', 'city', 'phone', 'gender', 'linkedIn', 'recruitment', 'country_id', 'policy_agreement', 'is_lecturer']}
        qcontext.update(other_fields)
        return qcontext
    
    def _prepare_signup_values(self, qcontext):
        values = super(RedirectedSignup, self)._prepare_signup_values(qcontext)
        values.update({ key: qcontext.get(key) for key in ('last_name', 'city', 'phone', 'gender', 'linkedIn', 'recruitment', 'country_id', 'policy_agreement', 'is_lecturer') })
        return values
